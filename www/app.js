...
/* ---------- model-CDN versies ---------- */
const TF = {
  coco: { url: 'https://cdn.jsdelivr.net/npm/@tensorflow-models/coco-ssd@2.2.3/dist/coco-ssd.min.js' },
  mnet: { url: 'https://cdn.jsdelivr.net/npm/@tensorflow-models/mobilenet@2.1.1/dist/mobilenet.min.js' }
};
...
async function ensureModel(engine){
  ...
  const p=(async()=>{
    if(engine==='coco'){ await loadScript(TF.coco.url); return await cocoSsd.load({base:'lite_mobilenet_v2'}); }
    else { await loadScript(TF.mnet.url); return await mobilenet.load({version:2,alpha:1.0}); }
  })();
  modelState[engine]=p;
...
function openTest(objectId, pool){
  ...
  testDetector=new Detector($('testVideo'),$('testCanvas'),obj,{
    ...
    onError:(w)=>{ $('testMsg').innerHTML=`<div>⚠️ ${w==='camera'?'Geen toegang tot de camera.':'Model kon niet laden. Check je verbinding.'}</div>`; $('testStatusText').textContent='Mislukt'; },
    onTick:(prog,info)=>{
